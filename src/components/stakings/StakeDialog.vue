<template>
  <Dialog :header="
    isRemovingStake ? 'Unstake your PRIM' : 'Stake PRIM to earn PRIM'
  " v-model:visible="displayDialog" contentClass="d-flex flex-column" dismissableMask modal>
    <div class="d-flex justify-content-between mb-2">
      ...
      <div class="d-flex align-items-center">
        <img class="item sq-2 rounded-circle" src="/images/logo.png" alt="PRIM" />
        <span class="ml-2">PRIM</span>
      </div>
    </div>
    ...
    <div class="jfPlkw" v-if="userNotEnoughToken">
      Insufficient PRIM balance
    </div>
    <div class="jfPlkw" v-if="inEligible">
      Minimum amount to participate is 2 billion PRIM tokens
    </div>
    ...
    <div class="d-flex justify-content-between mt-3" v-if="!isRemovingStake">
      ...
      <span v-if="annualRoi > 0">{{ formatNumber(annualRoi, 2, 4) }} PRIM</span>
      <span v-else>_</span>
    </div>
    <template #footer>
...
import usePrimStaking from "@/helpers/staking/usePrimStaking";
import { updateUserBalance, updateUserStakedBalance, updateUserStakingDetails } from "@/helpers/staking";
import usePrimUnStaking from "@/helpers/staking/usePrimUnStaking";
...
export default class StakeDialog extends mixins(Vue.with(Props), CommonMixin) {
  ...
  async handleConfirmClick() {
    if (
      ...
      return;
    const { onStake } = usePrimStaking(this.stakingData.lid);
    const { onUnstake } = usePrimUnStaking(this.stakingData.lid);
      ...
      if (receipt?.status) {
        if (this.isRemovingStake) {
          this.$store.commit("useToast", {
            severity: "success",
            summary: "Unstaked",
            detail: `Your PRIM earnings have also been harvested to your wallet!`,
          });
        } else {
          this.$store.commit("useToast", {
            severity: "success",
            summary: "Staked",
            detail: `Your PRIM funds have been staked!`,
          });
        }
