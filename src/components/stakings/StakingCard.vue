    ...
    <div class="card">
      <div class="card-header">
        <div class="d-flex align-items-end flex-wrap gap-1">
          <div class="stacked">
            <img class="item sq-3 rounded-circle" src="/images/logo.png" alt="PrimeVerse" />
          </div>
          <div class="text-left content">
            <h5>{{stakingData.tier}}</h5>
            <p style="color: #606060">Stake Prim</p>
          </div>
        </div>
      </div>
      <div class="card-body">
        ...
        <div v-if="stakingData.lockPlan == 'Semi-Flexible'" class="info-text">
          Unstaking before lock ends, results in a 50% fine from harvest earnings
        </div>
        <HarvestAction v-if="stakingData.tier == 'GOVERNOR TIER' && stakingData.userData?.totalReward > 0"
