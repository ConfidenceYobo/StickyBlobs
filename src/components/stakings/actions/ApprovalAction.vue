...
import { getPrimTokenAddress } from "@/utils/addressHelpers";
...
export default class ApprovalActions extends mixins(Vue.with(Props), CommonMixin) {
  ...
  async handleApprove() {
    const primContract = useERC20(getPrimTokenAddress() || "");
    const { handleApprove } = useApproveStaking(primContract);
    ...
    try {
      ...
      if (receipt?.status) {
        this.$store.commit("useToast", {
          ...
          detail: `You can now stake your PRIM!`,
        });
