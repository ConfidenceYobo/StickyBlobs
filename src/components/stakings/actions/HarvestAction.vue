...
export default class HarvestAction extends mixins(Vue.with(Props), CommonMixin) {
  ...
  async handleHarvestConfirm() {
    ...
    try {
      ...
      if (receipt?.status) {
        this.$store.state.toast.add({ severity: 'success', summary: 'Harvested', detail: `Your PRIM earnings have been sent to your wallet!`, life: 5000 });
